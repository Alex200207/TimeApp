...
export const FavoritesAside = ({
  ...
  return (
    <aside
      className={`
      fixed top-0 right-0 h-full z-50 ${isMobile ? "w-full" : "w-[23rem]"} ${
        isMobile ? "z-50" : "z-40"
      ...
      <ScrollArea className="h-[calc(100vh-48px)]">
        {favorites.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full p-4 text-center text-white">
            <Heart className="w-16 h-16 mb-4 text-white animate-pulse" />
            <h3 className="text-xl font-semibold mb-2">No hay favoritos aún</h3>
            <p className="text-s text-white max-w-xs">
              Agrega ciudades a tus favoritos para verlas aquí y acceder
              rápidamente a su información meteorológica.
            </p>
          </div>
        ) : (
                    ...
                    {isExpanded && (
                      <div className="mt-2 pt-2 border-t border-slate-700">
                        ...
                        <div className="grid grid-cols-2 gap-1 text-xs text-slate-400">
                          <div>
                            Sensación: {fav.weather?.main.feels_like.toFixed(1)}
                            °C
                          </div>
